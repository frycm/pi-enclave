...
class DockerCompiled implements CompiledProfile {
...
export class DockerBackend implements SandboxBackend {
	...
	private readonly options: Required<DockerBackendOptions>;
	private readonly privateDir: string;
	private readonly containers = new Set<OwnedContainer>();
	private readonly helpers = new Map<DockerCompiled, HelperFsClient>();
	private readonly profiles = new Set<DockerCompiled>();
	...
	constructor(options: DockerBackendOptions) {
		if (!IMAGE.test(options.image))
			throw new Error("pi-enclave: Docker image must be an immutable sha256 ID or repository@sha256 digest");
		if (process.platform !== "linux")
			throw new Error("pi-enclave: experimental Docker runner currently requires a Linux host");
		const binary = dockerPath(options.binary ?? "/usr/bin/docker");
	...
	private cliEnv(): NodeJS.ProcessEnv {
		return { PATH: "/usr/bin:/bin", HOME: this.privateDir, LANG: "C.UTF-8" };
	}
	...
	private async control(args: readonly string[]): Promise<string> {
		try {
			...
			throw new Error(
				`pi-enclave: Docker ${args[0]} failed: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}

	private assertActive(): void {
		if (this.disposed) throw new Error("pi-enclave: Docker backend was disposed");
	...
	private preflight(): Promise<string> {
		this.ready ??= (async () => {
			...
			if (
				...
				throw new Error(
					"pi-enclave: Docker requires a Linux daemon with seccomp and unmapped host UID support; rootless/userns mapping is not qualified yet",
				);
			}
			...
			if (
				...
				throw new Error("pi-enclave: Docker requires one Linux image without implicit VOLUME mounts");
			}
			return image.Id;
		})();
		return this.ready;
	...
	async compile(profile: Profile): Promise<CompiledProfile> {
		...
		const compiled = Object.freeze(new DockerCompiled(plan, await this.preflight()));
		this.assertActive();
		this.profiles.add(compiled);
		return compiled;
	}

	private checked(compiled: CompiledProfile): DockerCompiled {
		this.assertActive();
		if (!(compiled instanceof DockerCompiled) || !this.profiles.has(compiled))
			throw new Error("pi-enclave: foreign Docker profile");
		if (mountSnapshot(compiled.plan.observedPaths) !== compiled.plan.snapshot)
			throw new Error("pi-enclave: Docker mount topology changed; recompile before executing");
		return compiled;
	}

	private environment(compiled: DockerCompiled, parent: ChildEnv): ChildEnv {
	...
	private async create(
		...
		if (
			...
			throw new Error("pi-enclave: Docker cwd must be inside an explicit host root");
		const uid = process.getuid?.();
		const gid = process.getgid?.();
		if (!uid || gid === undefined || gid === 0)
			throw new Error("pi-enclave: Docker runner requires a non-root host UID/GID");
		...
		const create = (async () => {
			try {
				await this.control([
					...
					"--entrypoint",
					"/usr/bin/env",
					...mountArguments(compiled.plan, this.privateDir),
					...(helper
						? [
								"--mount",
								`type=bind,source=${this.privateDir}/helper.mjs,target=/opt/pi-enclave/helper.mjs,readonly,bind-recursive=disabled`,
							]
						: []),
					compiled.image,
					"-i",
	...
	private remove(owned: OwnedContainer): Promise<void> {
		owned.removal ??= (async () => {
			...
			const failure = new Error(
				`pi-enclave: Docker container cleanup could not be verified (${owned.name}): ${String(error)}`,
			);
	...
	async run(profile: CompiledProfile, request: RunRequest): Promise<RunResult> {
		...
		if (request.readCapability || request.writeCapability)
			throw new Error("pi-enclave: Docker capability profiles are not enabled in this experimental slice");
	...
	fs(profile: CompiledProfile): FsClient {
		...
		const client = new HelperFsClient({
			...
			spawnHelper: () => {
				if (!owned) throw new Error("pi-enclave: Docker helper container was not prepared");
				const child = this.attach(owned);
				child.once("close", () => {
					prepared = undefined;
					owned = undefined;
