...
import type { ContainerBackend } from "../../src/backend/container.ts";
import { DockerBackend } from "../../src/backend/docker.ts";
import { PodmanBackend } from "../../src/backend/podman.ts";
...
export function offlineBoundary(engine: "docker" | "podman") {
	...
	const cliArgs = (args: string[]) =>
		engine === "podman" && process.platform === "darwin"
			? ["machine", "ssh", machine, ["podman", "--remote=false", ...args].map(shellQuote).join(" ")]
			: args;

	describe.skipIf(!image)(`offline ${engine} boundary`, () => {
		...
		beforeEach(async () => {
			root = mkdtempSync(
				join(realpathSync(process.platform === "darwin" ? userInfo().homedir : tmpdir()), "enclave-live-"),
			);
			...
			backend =
				engine === "docker"
					? new DockerBackend({ image: image ?? "", binary })
					: new PodmanBackend({ image: image ?? "", binary, ...(process.platform === "darwin" ? { machine } : {}) });
...
try: socket.getaddrinfo('example.com',443);print('DNS_LEAK')
		...
		it("keeps resource limits active and image temporary directories read-only", async () => {
			const result = await sh(
				"cat /sys/fs/cgroup/memory.max /sys/fs/cgroup/memory.swap.max /sys/fs/cgroup/pids.max /sys/fs/cgroup/cpu.max",
			);
			expect(result.exitCode, result.output).toBe(0);
			const limits = result.output.trim().split("\n");
			expect(limits[0]).toBe("1073741824");
			expect(limits[1]).toBe("0");
			expect(limits[2]).toBe("256");
			const [quota, period] = (limits[3] ?? "").split(" ").map(Number);
			expect((quota ?? 0) / (period ?? 0)).toBe(2);
			for (const path of ["/run", "/var/tmp", "/etc"])
				expect((await sh(`touch ${path}/enclave-should-not-write`)).exitCode).not.toBe(0);
			expect((await sh("touch /tmp/ordinary /dev/shm/ordinary")).exitCode).toBe(0);
		});
		...
		it.each(["timeout", "abort"])("reaps a setsid descendant before returning from %s", async (kind) => {
			...
			const remaining = execFileSync(binary, cliArgs(["ps", "-aq", "--filter", `volume=${workspace}`]), {
				encoding: "utf8",
			}).trim();
